# Developer Guide: Sovereign HVAC Thermodynamics
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/hvac-thermo.git
cd hvac-thermo
...
```bash
export SBB_ASHRAE_EDITION=your_value_here
export SBB_CLIMATE_ZONE=your_value_here
export SBB_UNIT_SYSTEM=your_value_here
export SBB_REFRIGERANT_TYPE=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-hvac-thermo)
...
```bash
# Build
docker build -t sovereign-hvac-thermo:dev .

# Run with env file
docker run --env-file .env -p 8782:8782 sovereign-hvac-thermo:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-hvac-thermo
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8782`
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8782` in use | `lsof -i :8782` | Kill PID or change `SERVICE_PORT` |
